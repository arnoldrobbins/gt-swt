      SUBROUTINE SYNERR(MESSA0,WARNF0)
      INTEGER MESSA0(1)
      INTEGER WARNF0
     ...
     *ME0
      ...
      DATA AAAAB0/170,181,233,0/
      DATA AAAAC0/160,168,170,243,169,186,160,0/
      DATA AAAAD0/215,225,242,238,233,238,231,186,160,170,240,0/
      DATA AAAAE0/188,206,197,215,204,201,206,197,190,0/
      DATA AAAAF0/188,197,207,198,190,0/
      DATA AAAAG0/170,243,170,243,174,170,238,0/
      DATA AAAAH0/170,243,167,170,243,167,160,170,243,174,170,238,0/
      DATA AAAAI0/170,243,167,170,243,167,170,238,170,177,176,248,170,24
     *3,174,170,238,0/
      DATA AAAAJ0/212,239,239,160,237,225,238,249,160,229,242,242,239,24
     *2,243,160,173,173,160,231,233,246,233,238,231,160,245,240,174,0/
      NL=1
      I=1
      GOTO 10002
10000 I=I+1
10002 IF((I.GE.LEVEL0))GOTO 10001
        NL=NL+(ENCODE(NUMS(NL),102-NL,AAAAA0,LINEN0(I)))
      GOTO 10000
10001 NL=NL+(ENCODE(NUMS(NL),102-NL,AAAAB0,SYMLI0))
      NL=NL+(ENCODE(NUMS(NL),102-NL,AAAAC0,MODUL0))
      IF((WARNF0.NE.1))GOTO 10003
        ML=ENCODE(MSG,102,AAAAD0,MESSA0)
        GOTO 10004
10003   ML=PTOC(MESSA0,174,MSG,102)
10004 EL=LENGTH(ERROR0)
      IF((EL.NE.0))GOTO 10005
        IF((SYMBO0.NE.138))GOTO 10006
          EL=CTOC(AAAAE0,ERROR0,200)
          GOTO 10007
10006     IF((SYMBO0.NE.-1))GOTO 10008
            EL=CTOC(AAAAF0,ERROR0,200)
            GOTO 10009
...
10009   CONTINUE
10007 CONTINUE
10005 IF((EL.NE.0))GOTO 10011
        CALL PRINT(-15,AAAAG0,NUMS,MSG)
        GOTO 10012
10011   IF((EL+NL+ML.GT.73))GOTO 10013
          CALL PRINT(-15,AAAAH0,NUMS,ERROR0,MSG)
          GOTO 10014
10013     CALL PRINT(-15,AAAAI0,NUMS,ERROR0,MSG)
10014 CONTINUE
10012 IF((WARNF0.NE.0))GOTO 10015
        NERRS0=NERRS0+(1)
10015 IF((NERRS0.LE.50))GOTO 10016
        CALL ERROR(AAAAJ0)
10016 ERROR0(1)=0
      RETURN
      END
     ...
     *ME0
      SEMSP0=SEMSP0+(SIZE)
      IF((SEMSP0.LE.300))GOTO 10017
        CALL FATAL0('Semantic stack overflow.')
10017 RETURN
      END
      SUBROUTINE SSDEA0(SIZE)
      INTEGER SIZE
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *ME0
      SEMSP0=SEMSP0-(SIZE)
      IF((SEMSP0.GE.1))GOTO 10018
        CALL FATAL0('Semantic stack underflow.')
10018 RETURN
      END
      SUBROUTINE CSPUSH(TYPE)
      INTEGER TYPE
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *ME0
      CTLSP0=CTLSP0+(1)
      IF((CTLSP0.LE.50-1))GOTO 10019
        CALL FATAL0('Control stack overflow.')
10019 CTLSK0(CTLSP0)=TYPE
      RETURN
      END
      SUBROUTINE CSPOP
      INTEGER SYMTE0(200),NSYMT0(200)
     ...
     *ME0
      CTLSP0=CTLSP0-(1)
      IF((CTLSP0.GE.1))GOTO 10020
        CALL FATAL0('Control stack underflow.')
10020 RETURN
      END
      SUBROUTINE SKIPTO(TOKEN)
      INTEGER TOKEN
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *0
      ...
      COMMON /MISCOM/MODUL0,ERROR0,A$BUF,OUTFI0,OUTFP0,CKFIL0,NERRS0,FNA
     *ME0
10021 IF((SYMBO0.EQ.TOKEN))GOTO 10022
      IF((SYMBO0.EQ.187))GOTO 10022
      IF((SYMBO0.EQ.-1))GOTO 10022
        CALL GETSYM
      GOTO 10021
10022 RETURN
      END
     ...
     *0
      ...
      COMMON /MISCOM/MODUL0,ERROR0,A$BUF,OUTFI0,OUTFP0,CKFIL0,NERRS0,FNA
     *ME0
      IF((EXPSP0.LE.100))GOTO 10023
        CALL FATAL0('Expression stack overflow.')
10023 EXPSK0(EXPSP0)=P
      EXPSP0=EXPSP0+(1)
      RETURN
      END
      INTEGER FUNCTION ESPOP(P)
      INTEGER P
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *ME0
      EXPSP0=EXPSP0-(1)
      IF((EXPSP0.GE.1))GOTO 10024
        CALL SYNERR('Expression stack underflow.',0)
        P=0
        EXPSP0=1
        GOTO 10025
10024   P=EXPSK0(EXPSP0)
10025 ESPOP=P
      RETURN
      END
      INTEGER FUNCTION ESTOP(P)
      INTEGER P
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *0
      ...
      COMMON /MISCOM/MODUL0,ERROR0,A$BUF,OUTFI0,OUTFP0,CKFIL0,NERRS0,FNA
     *ME0
      IF((MEMAA0(P+2).EQ.6))GOTO 10027
      IF((MEMAA0(P+2).EQ.3))GOTO 10027
      IF((MEMAA0(MEMAA0(P+1)+3).EQ.15))GOTO 10027
      IF((MEMAA0(P+4).NE.-1))GOTO 10027
      GOTO 10026
10027   ISSTO0=0
        RETURN
10026 ISSTO0=1
     ...
     *0
      ...
      COMMON /MISCOM/MODUL0,ERROR0,A$BUF,OUTFI0,OUTFP0,CKFIL0,NERRS0,FNA
     *ME0
      IF((MEMAA0(P).NE.8))GOTO 10030
      IF((MEMAA0(P+5).NE.1))GOTO 10030
      GOTO 10029
10030 IF((MEMAA0(P).NE.1))GOTO 10028
      IF((MEMAA0(MEMAA0(P+1)+3).EQ.13))GOTO 10028
      GOTO 10029
10029   ISLVA0=1
        RETURN
10028 ISLVA0=0
     ...
     *ME0
      INTEGER AAAAK0
      AAAAK0=MEMAA0(MEMAA0(P+1)+3)
      GOTO 10032
10033   ISARI0=1
        RETURN
10032 GOTO(10033,10033,10033,10033,10033,10033,10033,10033,10033,10033, 
     *    10034,10034,10034,10033),AAAAK0
10034 CONTINUE
      ISARI0=0
      RETURN
      END
      INTEGER FUNCTION ISINT(P)
      INTEGER P
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *ME0
      INTEGER AAAAL0
      AAAAL0=MEMAA0(MEMAA0(P+1)+3)
      GOTO 10035
10036   ISINT=1
        RETURN
10035 GOTO(10036,10036,10036,10036,10036,10036,10036,10036,10037,10037, 
     *    10037,10037,10037,10036),AAAAL0
10037 CONTINUE
      ISINT=0
      RETURN
      END
      INTEGER FUNCTION ISPOI0(P)
      INTEGER P
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *0
      ...
      COMMON /MISCOM/MODUL0,ERROR0,A$BUF,OUTFI0,OUTFP0,CKFIL0,NERRS0,FNA
     *ME0
      IF((MEMAA0(MEMAA0(P+1)+3).NE.12))GOTO 10038
        ISPOI0=1
        RETURN
10038 ISPOI0=0
      RETURN
      END
      INTEGER FUNCTION ISCON0(V)
      INTEGER V
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *0
      ...
      COMMON /MISCOM/MODUL0,ERROR0,A$BUF,OUTFI0,OUTFP0,CKFIL0,NERRS0,FNA
     *ME0
      IF((MEMAA0(V).NE.3))GOTO 10039
        ISCON0=1
        RETURN
10039 ISCON0=0
      RETURN
      END
      INTEGER FUNCTION ISAGG0(MP)
      INTEGER MP
      INTEGER SYMTE0(200),NSYMT0(200)
      INTEGER SYMLE0,SYMBO0,SYMLI0,NSYML0,NSYMB0,NSYMM0
      INTEGER SYMPT0,NSYMP0
     ...
     *0
      ...
      COMMON /MISCOM/MODUL0,ERROR0,A$BUF,OUTFI0,OUTFP0,CKFIL0,NERRS0,FNA
     *ME0
      IF((MEMAA0(MP+3).EQ.16))GOTO 10041
      IF((MEMAA0(MP+3).EQ.17))GOTO 10041
      GOTO 10040
10041   ISAGG0=1
        RETURN
10040 ISAGG0=0
      RETURN
      END
      SUBROUTINE PUTLO0(D,S)
      INTEGER * 4 D,S
      D=S
      RETURN
      END
      INTEGER * 4 FUNCTION GETLO0(S)
      INTEGER * 4 S
      GETLO0=S
      RETURN
      END
