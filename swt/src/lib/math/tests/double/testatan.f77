C     TEST --- Subroutine to test the double precision ATAN function
C
C     Eugene Spafford
...
C
      INTEGER II
      DOUBLE PRECISION BETAP,EM,EXPON,OB32,SUM,XSQ,ZATAN
      EXTERNAL ZATAN
      CHARACTER*6 RNAME
C
C
...
C
          ELSEIF (J .EQ. 2) THEN
              Y = X-.0625D0
              Y = Y/(ONE+X*A)
              ZZ = (ZATAN(Y)-8.1190004042651526021D-5)+OB32
              ZZ = ZZ+OB32
C
          ELSE
              Z = Z+Z
