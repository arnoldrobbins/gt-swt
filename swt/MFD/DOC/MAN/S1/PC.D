[cc]mc |
.hd pc "interface to Primos Pascal compiler" 08/27/84
[cc]mc
pc {-<option>[<level>]} <input file>
   ...
   <option> ::= c | d | e | f | h | k | m | n |
                o | q | r | s | u | v | w | x
.ds
'Pc' serves as the Subsystem interface to the Primos Pascal compiler
(PASCAL).
...
Primos PASCAL command and causes it to be executed.
.sp
.bf
Options
.sp
The general structure of an 'pc' option is a single letter, possibly
...
.op s 0 1 0 "Check for use of non-standard features"
.sp
Level 0 allows all features of Prime Pascal.
.sp
Level 1 generates a syntax error for the use of any feature
not in the proposed ANSI standard.
.op u 0 1 0 "Generate external procedure definition"
.sp
...
code produced by the PASCAL compiler.  This option
...
to ".b".
For example, if the input filename is "prog.p", the binary file will be
"prog.b";
...
its suffix to ".l".
For example, if the input filename is "gonzo.p", the listing file will
be "gonzo.l";
...
a serious error in the operation of 'pc' that should be reported
