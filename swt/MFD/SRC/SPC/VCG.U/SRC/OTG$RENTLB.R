# otg$rentlb --- enter a relative entry definition group
#
#               address is LB% + reloc offset + offset

   subroutine otg$rentlb (name, offset)
   character name (ARB)
   integer offset
   ...
   group_data (1) = RELENTLB_GROUP * BIT8 + 1
   group_data (2) = offset
   ...
   group_data (1) += len / 2
DB call print (TTY, "otg$rentlb: ENT *,#s LB%+reloc+'*,-8i*n"s,
DB                      len, name, offset)
