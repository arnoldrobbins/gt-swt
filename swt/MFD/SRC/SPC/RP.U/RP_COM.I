# Common areas for Ratfor preprocessor
  ...
  # Select Statement Generation
   ...
   common /selgen/ Scvalue, Sclabel, Slt, Result
  ...
  # Goto Management:
   integer Dispatch_flag, Last_dispatch_flag, Xgo_from (MAXGOHASH),
      Xgo_to (MAXGOHASH), Lgo_line (MAXGOLIST), Lgo_pos (MAXGOLIST),
      Lgo_stmt (MAXGOLIST), Lgo_lp

   common /gocom/ Dispatch_flag, Last_dispatch_flag, Xgo_from,
      Xgo_to, Lgo_line, Lgo_pos, Lgo_stmt, Lgo_lp


  # Miscellaneous Junk:
   character Module_name (MAXTOK), Module_long_name (MAXTOK),
      Error_sym (MAXTOK), Tlit_char (MAXCHARVAL), Tlit_eos
   integer Curlab, Brace_count, Indent, First_stmt, Spnum,
      Last_num_out, Code_line_num
   file_des Prof_dict_file
   ARG_DECL

   common /miscom/ Module_name, Curlab, Brace_count, Indent,
      Module_long_name, First_stmt, Prof_dict_file, Spnum,
      Error_sym, ARG_BUF, Last_num_out, Code_line_num,
      Tlit_char, Tlit_eos
