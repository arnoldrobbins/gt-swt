...
# fsubc --- Prime DBMS Fortran subschema compiler interface

   include ARGUMENT_DEFS
   include COMPILER_DEFS

# options understood by FSUBS, exclusive of file manipulation:

   define (ERRLIST,1)

# table of FSUBS options, their current state, their default value,
...
#     on compiler options, and restrictions on other interface options.
   ...
   if (parscl ("v<oi> l<os>z<os>"s, ARG_BUF) ~= OK) {
      call error ("Usage: fsubc {-v[<level>]} <file> " _
                               "{-l [<file>]} [-z <string>]"p)
      }
   ...
   call print (STDOUT, "fsubs -i *s -l *s"p, inpf, listf)
