...
# cdmlc --- Prime DBMS Cobol DML preprocessor interface

   include ARGUMENT_DEFS
   include COMPILER_DEFS

# options understood by CDML, exclusive of file manipulation:

   define (ERRLIST,1)

# table of CDML options, their current state, their default value,
...
#     on compiler options, and restrictions on other interface options.
   ...
   if (parscl ("v<oi> b<os>l<os>z<os>"s, ARG_BUF) ~= OK) {
      call error ("Usage: cdmlc {-v[<level>]} <file> " _
                               "{-(b|l) [<file>]} [-z <string>]"p)
      }
   ...
   if (comopt (ARG_BUF, cpos, ctab, ipos, itab) == ERR
         || comfn (ARG_BUF, inpf, ".cob,.cobol"s,
                           listf, ".dl"s, binf, ".dcob"s) == ERR) {
      call seterr (1000)
      stop
      }
   ...
   if (equal (binf, "no") == YES)
      call error ("Sorry, output file must go to a disk file"p)

   call print (STDOUT, "cdml -i *s -o *s -l *s"p, inpf, binf, listf)
