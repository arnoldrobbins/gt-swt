...
# csubc --- Prime DBMS Cobol subschema compiler interface

   include ARGUMENT_DEFS
   include COMPILER_DEFS

# options understood by CSUBS, exclusive of file manipulation:

   define (ERRLIST,1)

# table of CSUBS options, their current state, their default value,
...
#     on compiler options, and restrictions on other interface options.
   ...
   if (parscl ("v<oi> l<os>z<os>"s, ARG_BUF) ~= OK) {
      call error ("Usage: csubc {-v[<level>]} <file> " _
                               "{-l [<file>]} [-z <string>]"p)
      }
   ...
   call print (STDOUT, "csubs -i *s -l *s"p, inpf, listf)
