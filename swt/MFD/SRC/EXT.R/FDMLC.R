...
# fdmlc --- Prime DBMS Fortran DML preprocessor interface

   include ARGUMENT_DEFS
   include COMPILER_DEFS

# options understood by FDML, exclusive of file manipulation:

   define (ERRLIST,1)

# table of FDML options, their current state, their default value,
...
#     on compiler options, and restrictions on other interface options.
   ...
   if (parscl ("v<oi> b<os>l<os>z<os>"s, ARG_BUF) ~= OK) {
      call error ("Usage: fdmlc {-v[<level>]} <file> " _
                               "{-(b|l) [<file>]} [-z <string>]"p)
      }
   ...
   if (comopt (ARG_BUF, cpos, ctab, ipos, itab) == ERR
         || comfn (ARG_BUF, inpf, ".f,.f77,.ftn"s,
                           listf, ".dl"s, binf, ".df"s) == ERR) {
      call seterr (1000)
      stop
      }
   ...
   call print (STDOUT, "fdml -i *s -o *s -l *s"p, inpf, binf, listf)
