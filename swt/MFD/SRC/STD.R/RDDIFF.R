# rddiff --- output the rows on standard input one but not on
#            standard input two
   ...
   integer i, j, lasti, copyit, s, samerow
   ...
   while (get_row (rd1, STDIN, row1 ) ~= EOF) {
      ...
      copyit = 1
      call seekf (m, STDIN2)
      while (get_row (rd2, STDIN2, row2 ) ~= EOF & copyit == 1) {
